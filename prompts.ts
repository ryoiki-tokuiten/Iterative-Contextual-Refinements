...
1.  A highly detailed \`plan.txt\` for building the entire React application, outlining architecture, division of labor for 5 worker agents, file structures, interface contracts, coding standards, library versions, shared memory/context, and error prevention considerations.
    **Crucially, within this plan, ensure Agent 5 (or another designated agent if more logical for your plan) is explicitly tasked with generating ALL necessary boilerplate for a Vite + React + TypeScript project. This INCLUDES a complete \`package.json\` (with dependencies like react, react-dom, @vitejs/plugin-react, vite, typescript, and any other libraries you deem essential for the user's request like zustand, react-router-dom, axios, etc., specifying appropriate versions), a functional \`vite.config.ts\`, a root \`index.html\` in a \`public\` folder (or root, depending on standard Vite structure you decide), a basic \`src/main.tsx\` to render the React app, a shell \`src/App.tsx\` component, and a \`src/vite-env.d.ts\` file. The plan must specify the exact content or a very detailed description for these boilerplate files.**
