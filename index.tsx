...
type ApplicationMode = 'website' | 'creative' | 'math' | 'agent';
...
function updateControlsState() {
    ...
    isGenerating = anyPipelineRunningOrStopping || mathPipelineRunningOrStopping;
    ...
    if (modelSelectElement) modelSelectElement.disabled = isGenerating || currentMode === 'math'; // Math uses fixed model
    if (pipelineSelectorsContainer) {
        const disableSelectors = isGenerating || currentMode === 'math';
        ...
        if (pipelineSelectLabel) {
             pipelineSelectLabel.textContent = currentMode === 'math' ? 'Math mode uses a fixed configuration.' : 'Select Variants to Run (Temperature):';
    ...
    const currentPromptTextareas = 
        ...
        currentMode === 'agent' ? customPromptTextareasAgent : null;
...
function exportConfiguration() {
    ...
    const config: ExportedConfig = {
        ...
        activePipelineId: (currentMode !== 'math') ? activePipelineId : null,
...
function handleImportConfiguration(event: Event) {
    ...
    reader.onload = (e) => {
        try {
            ...
            // Re-create math and agent prompts using current constants and imported templates
